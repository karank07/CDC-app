...
import { getPatientList, logout, getPatientListForDr, getAppointmentList, getAppointmentListDr } from "../../api/Api";
...
const Appointments = ({ history }) => {
  ...
  const fetchMyAPI = async () => {
    let response;
    response = await getAppointmentListDr();
    await setState({ ...state, appointmentList: response });
  };
  useEffect(() => {
    fetchMyAPI();
  }, []);
        ...
        <Grid container direction="row" style={{ marginTop: "4%", height: "80%" }}>
          ...
          <Grid
            ...
            <Typography variant="h6" gutterBottom className={classes.title}>
              You have {state.appointmentList.length} upcoming appointments.
            </Typography>
          </Grid>
          ...
          {state.appointmentList.length > 0 && (
            <Grid item sm={11} style={{ height: "100%" }}>
              <Grid
                container
                justify="center"
                alignItems="center"
                style={{ backgroundColor: "#F2F6F8", borderRadius: 30, marginTop: 20 }}>
                <TableContainer component={Paper} elevation={0} style={{ borderRadius: 15, margin: 20, height: 650 }}>
                  <Table
                    stickyHeader
